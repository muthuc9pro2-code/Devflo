...
import ijson
from app.core.processing_config import JSON_STREAM_BUFFER_BYTES, MAX_DIAGNOSTIC_RECORD_BYTES
...
class _OtelCapture:
    ...
    def consume(self, prefix: str, event: str, value: Any) -> None:
        ...
        if relative == 'attributes.item' and event == 'end_map':
            self._store_attribute(self.attributes, self._attribute_key, self._attribute_value)
        ...
        if relative == 'events.item.attributes.item.key' and event == 'string':
            ...
            return
        if relative.startswith('events.item.attributes.item.value.') and event in SCALAR_EVENTS:
            self._event_attribute_value = value
            return
        if relative == 'events.item.attributes.item' and event == 'end_map':
            if self._event is not None:
                self._store_attribute(self._event['attributes'], self._event_attribute_key, self._event_attribute_value)
        ...
        if relative.startswith('events.item.') and event in SCALAR_EVENTS:
            ...
            return
        if event in SCALAR_EVENTS and relative and (not relative.startswith('attributes.')):
            self._store_attribute(self.fields, relative, value)

    def _store_attribute(self, target: dict[str, Any], key: str | None, value: Any) -> None:
...
def stream_otlp_events(*, file_path: str, source_file: str, skip_records: int=0, global_line_number: int=0) -> Iterator[ArtifactEvent]:
    from .diagnostic_adapters import ArtifactEvent
...
def _normalize_log_record(capture: _OtelCapture, *, line_number: int, source_file: str, resource_attributes: dict[str, Any], scope_name: str | None):
    attributes = capture.attributes
    message = _first(capture.fields, 'body.stringValue', 'body.string_value', 'body.intValue', 'body.int_value', 'body.doubleValue', 'body.double_value', 'body.boolValue', 'body.bool_value', 'body') or attributes.get('exception.message') or 'OpenTelemetry log record'
    defaults = _resource_defaults(resource_attributes)
    defaults.update(timestamp=_first(capture.fields, 'timeUnixNano', 'time_unix_nano', 'observedTimeUnixNano', 'observed_time_unix_nano'), level=normalize_otel_severity(_first(capture.fields, 'severityText', 'severity_text', 'severityNumber', 'severity_number')), trace_id=_first(capture.fields, 'traceId', 'trace_id'), span_id=_first(capture.fields, 'spanId', 'span_id'), request_id=_first_attribute(attributes, 'request.id', 'http.request.id', 'correlation.id', 'aws.request_id'), module=scope_name, exception_type=attributes.get('exception.type'), exception_message=attributes.get('exception.message'), endpoint=_first_attribute(attributes, 'http.route', 'url.path', 'http.target'), http_status=_first_attribute(attributes, 'http.response.status_code', 'http.status_code'))
    return normalize_text_event(str(message), line_number, source_file=source_file, source_format=ArtifactFormat.OPENTELEMETRY.value, defaults=defaults)

def _normalize_span(capture: _OtelCapture, *, line_number: int, source_file: str, resource_attributes: dict[str, Any], scope_name: str | None):
    attributes = capture.attributes
    exception_event = next((item for item in capture.events if str(item.get('name', '')).lower() == 'exception'), None)
    exception_attributes = exception_event.get('attributes', {}) if exception_event else {}
    ...
    raw_message = exception_message or status_message or f'OpenTelemetry span: {span_name}'
    status_is_error = str(status_code).upper() in {'2', 'ERROR', 'STATUS_CODE_ERROR'}
    defaults = _resource_defaults(resource_attributes)
    defaults.update(timestamp=_first(capture.fields, 'startTimeUnixNano', 'start_time_unix_nano'), level='ERROR' if status_is_error or exception_event else 'INFO', trace_id=_first(capture.fields, 'traceId', 'trace_id'), span_id=_first(capture.fields, 'spanId', 'span_id'), parent_span_id=_first(capture.fields, 'parentSpanId', 'parent_span_id'), request_id=_first_attribute(attributes, 'request.id', 'http.request.id', 'correlation.id'), module=scope_name, exception_type=exception_attributes.get('exception.type'), exception_message=exception_message, endpoint=_first_attribute(attributes, 'http.route', 'url.path', 'http.target', 'rpc.method'), http_status=_first_attribute(attributes, 'http.response.status_code', 'http.status_code'))
    return normalize_text_event(str(raw_message), line_number, source_file=source_file, source_format=ArtifactFormat.OPENTELEMETRY.value, defaults=defaults)

def _resource_defaults(attributes: dict[str, Any]) -> dict[str, Any]:
    return {'service': _first_attribute(attributes, 'service.name', 'faas.name'), 'host': _first_attribute(attributes, 'host.name', 'host.id', 'cloud.availability_zone'), 'container': _first_attribute(attributes, 'container.name', 'container.id'), 'pod': _first_attribute(attributes, 'k8s.pod.name', 'k8s.pod.uid')}
