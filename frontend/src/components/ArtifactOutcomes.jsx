...
function artifactPresentation(artifact) {
  ...
  if (artifact.status === 'resource_limited') return { label: 'Resource limit exceeded', tone: 'warning', symbol: '!' }
  if (artifact.status === 'processing_error') return { label: 'Could not analyze', tone: 'danger', symbol: '×' }
              ...
              <div className="artifact-copy">
                ...
                <div className="artifact-meta">
                  ...
                  {Number.isFinite(Number(artifact.evidence_count)) && Number(artifact.evidence_count) > 0 && (
                    <span>
                      {artifact.evidence_count} evidence {Number(artifact.evidence_count) === 1 ? 'record' : 'records'}
                    </span>
                  )}
                </div>
              </div>
