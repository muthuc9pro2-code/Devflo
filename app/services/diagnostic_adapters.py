...
from app.core.processing_config import (
    ARTIFACT_DETECTION_SAMPLE_BYTES,
...
class _BoundedStructuredCapture:
    ...
    def consume(self, prefix: str, event: str, value: Any) -> None:
        ...
        existing_size = (
            len(str(existing_value).encode("utf-8", errors="replace"))
    ...
    def result(self) -> dict[str, Any]:
        if "message" not in self.values:
            ...
            if exception_message is not None:
                ...
                self.values["message"] = " ".join(
                    str(item) for item in (method, endpoint, status) if item is not None
                )
            else:
...
def stream_artifact_events(
    *,
    ...
    if artifact_format == ArtifactFormat.OPENTELEMETRY:
        ...
        yield from stream_otlp_events(
            file_path=file_path,
            source_file=source_file,
            skip_records=start_artifact_line,
    ...
    if artifact_format == ArtifactFormat.CLOUD_GATEWAY:
        ...
        if cloudfront_fields is not None:
            yield from _stream_cloudfront_events(
                file_path=file_path,
                source_file=source_file,
                fields=cloudfront_fields,
                start_offset=start_offset,
                start_artifact_line=start_artifact_line,
                global_line_number=global_line_number,
            )
            return
    ...
    if artifact_format in {
        ...
        and _artifact_starts_with_json(file_path)
    ):
        yield from _stream_json_events(
            file_path=file_path,
    ...
    yield from _stream_text_events(
        file_path=file_path,
        artifact_format=artifact_format,
        source_file=source_file,
        start_offset=start_offset,
...
def _stream_cloudfront_events(
    *,
    file_path: str,
    source_file: str,
    fields: list[str],
    start_offset: int,
    start_artifact_line: int,
    global_line_number: int,
) -> Iterator[ArtifactEvent]:
    """Stream CloudFront TSV rows using a schema recoverable after resume."""
    ...
    for line, end_offset in stream_text_lines(file_path, start_offset=start_offset):
        ...
        values = line.split("\t")
        if len(values) == 1:
            # CloudFront emits tabs, but accepting whitespace-delimited test or
            # exported rows costs no unbounded buffering and is a safe fallback.
            values = line.split()
        data = {key.lower(): value for key, value in zip(fields, values, strict=False)}
        event = _normalize_cloudfront_event(
            line,
            global_line,
            source_file,
            data,
        ...
        yield ArtifactEvent(
            ...
            batch_size_bytes=max(
                ...
                estimate_parsed_event_size_bytes(event),
                1,
            ),
        )
...
def _read_cloudfront_fields(file_path: str) -> list[str] | None:
    """Read only the bounded format-detection prefix to recover the TSV schema."""
...
def _cloudfront_fields_from_line(line: str) -> list[str] | None:
    ...
    if not {
        ...
        "sc-status",
        "cs-uri-stem",
    } <= lowered or not lowered.intersection(
...
def _stream_text_events(
    *,
    ...
    for line, end_offset in stream_text_lines(file_path, start_offset=start_offset):
        ...
        cri_match = (
            CRI_RE.search(line) if artifact_format == ArtifactFormat.CONTAINER else None
        )
        ...
        if cri_match is not None:
            if pending is not None and pending.multiline_kind == "cri_partial":
                ...
                same_stream = pending_match is not None and pending_match.group(
                    "stream"
                ) == cri_match.group("stream")
                fits_record_bound = (
                    pending.size_bytes + line_size + 1 <= MAX_DIAGNOSTIC_RECORD_BYTES
                )

                if same_stream and fits_record_bound:
                    # CRI ``P`` chunks are byte fragments of one logical log
                    # line. The physical newlines delimit CRI records and are
                    # not part of the reconstructed diagnostic message.
                    ...
                    yield _build_text_artifact_event(
                        pending,
                        artifact_format=artifact_format,
                        source_file=source_file,
                    )
                ...
                # A stream switch or the configured record bound terminates
                # the current partial capture safely. The current fragment is
                # then handled as the start (or end) of another bounded event.
                yield _build_text_artifact_event(
                    pending,
                    artifact_format=artifact_format,
                    source_file=source_file,
            ...
            cri_record = _PendingTextRecord(
                ...
                multiline_kind=(
                    "cri_partial" if cri_match.group("flag") == "P" else None
                ),
            )
        ...
        if pending is not None and _is_multiline_continuation(
            pending,
            line,
        ):
        ...
        pending = _PendingTextRecord(
            ...
            end_offset=end_offset,
            size_bytes=line_size,
            multiline_kind=_multiline_kind(artifact_format, line),
        )
...
def _build_text_artifact_event(
    ...
    *,
    ...
    if artifact_format == ArtifactFormat.WEB_SERVER:
        ...
        elif artifact_format == ArtifactFormat.DATABASE and (
            "slow query" in lowered or "query_time:" in lowered
        ):
            defaults["level"] = "WARNING"
        elif (
            artifact_format == ArtifactFormat.STACK_TRACE
            and record.multiline_kind == "crash_report"
        ):
            defaults["level"] = "ERROR"
...
def _stream_json_events(
    *,
    ...
    yield from _stream_json_document(
        file_path=file_path,
        artifact_format=artifact_format,
        source_file=source_file,
        skip_records=start_artifact_line,
    ...
    *,
    ...
    for line, end_offset in stream_text_lines(file_path, start_offset=start_offset):
        ...
        try:
            ...
            event = normalize_structured_event(
                data,
                global_line,
                source_file=source_file,
                source_format=artifact_format.value,
    ...
    *,
    ...
    try:
        with open(file_path, "rb") as stream:
            ...
            for prefix, event, value in ijson.parse(
                ...
                if capture is not None:
                    ...
                    continue
                elif prefix == target_prefix and event in {
                    "string",
                    "number",
                    "boolean",
                    "null",
        ...
        yield from _stream_text_events(
            ...
            source_file=source_file,
            start_offset=start_offset,
            start_artifact_line=skip_records,
            global_line_number=global_line_number,
        )
...
def _json_item_prefix(file_path: str, artifact_format: ArtifactFormat) -> str:
    ...
    collection_match = re.search(
        r'"(?P<key>logEvents|records|Records|logs)"\s*:\s*\[',
        sample,
    )
    if collection_match:
...
def _is_json_lines(
    ...
    if artifact_format == ArtifactFormat.BROWSER:
        return False
    return not any(
        isinstance(first_value.get(key), list)
        for key in ("logEvents", "records", "Records", "logs")
    )


def _artifact_starts_with_json(file_path: str) -> bool:
    with open(file_path, "rb") as stream:
        return _json_prefix(stream.read(ARTIFACT_DETECTION_SAMPLE_BYTES)).startswith(
            (b"{", b"[")
        )
...
def _structured_canonical_key(relative: str) -> str | None:
    ...
    leaf = re.sub(r"[^a-z0-9]", "", relative.rsplit(".", 1)[-1].lower())

    if normalized.endswith(("errormessage", "exceptionmessage")) or leaf in {
        "error",
        "exception",
    }:
    ...
    if normalized.endswith(
        ("requesturl", "requestpath", "resourcepath", "urlpath", "httproute")
    ) or leaf in {
        "endpoint",
        "route",
        "routekey",
...
_STRUCTURED_ALIAS_ORDER = {
    "message": ("message", "msg", "log", "@message", "body", "event.message"),
    "timestamp": (
        "timestamp",
        "@timestamp",
        "time",
    ...
    "parent_span_id": ("parent_span_id", "parentspanid", "span.parent_id"),
    "request_id": (
        "request_id",
        "requestid",
        "correlation_id",
    ...
    "module": ("module", "logger", "logger_name", "loggername", "scope.name"),
    "host": (
        "host.name",
        "hostname",
        "host",
    ...
    "pod": ("pod", "pod_name", "podname", "kubernetes.pod_name"),
    "exception_type": (
        "exception_type",
        "exceptiontype",
        "exception.type",
    ...
    "status": ("status", "state"),
    "_method": ("requestmethod", "method", "httpmethod"),
}


def _structured_alias_priority(canonical_key: str, relative: str) -> int:
    normalized_path = ".".join(
        re.sub(r"[^a-z0-9_@-]", "", part.lower()) for part in relative.split(".")
    )
...
def _multiline_kind(
    ...
    if "panic:" in lowered:
        return "panic_stack"
    if artifact_format == ArtifactFormat.STACK_TRACE and EXCEPTION_PATTERN.match(
        line.strip()
    ):
        return "language_stack"
    if artifact_format == ArtifactFormat.DATABASE and line.lstrip().startswith(
        ("# Time:", "Time:")
    ):
        return "database_block"
    if artifact_format == ArtifactFormat.STACK_TRACE and any(
        marker in lowered for marker in ("crash report", "core dumped", "fatal signal")
    ):
        return "crash_report"
    return None
...
def _is_multiline_continuation(
    ...
    if record.multiline_kind == "java_stack":
        return bool(
            not stripped
            or line.startswith((" ", "\t"))
            or stripped.startswith(("at ", "Caused by:", "Suppressed:"))
        )
    if record.multiline_kind == "language_stack":
        return bool(
            not stripped
            or line.startswith((" ", "\t"))
            or stripped.startswith(("at ", "File ", "Caused by:", "Suppressed:"))
        )
    ...
    return bool(
        ...
        or stripped
        == "During handling of the above exception, another exception occurred:"
        or EXCEPTION_PATTERN.search(stripped)
    )
...
WEB_ERROR_RE = re.compile(
    r"^(?P<time>\d{4}/\d{2}/\d{2}\s+\d{2}:\d{2}:\d{2})\s+"
    r"\[(?P<level>[^\]]+)\]",
    re.IGNORECASE,
)
APACHE_ERROR_RE = re.compile(
    r"^\[(?P<time>[A-Z][a-z]{2}\s+[A-Z][a-z]{2}\s+\d{1,2}\s+"
...
def _normalize_web_event(
    raw_text: str, line_number: int, source_file: str
) -> ParsedEvent:
    ...
    if access_match:
        ...
        defaults.update(
            timestamp=access_match.group("time"),
            host=access_match.group("host"),
            endpoint=access_match.group("endpoint"),
            http_status=status,
        ...
        if error_match:
            defaults["timestamp"] = parse_timestamp(
                error_match.group("time").replace("/", "-", 2)
            )
    ...
    return normalize_text_event(
        ...
        line_number,
        source_file=source_file,
        source_format=ArtifactFormat.WEB_SERVER.value,
        defaults=defaults,
    )


SYSLOG_5424_RE = re.compile(
    r"^<(?P<pri>\d{1,3})>\d+\s+(?P<time>\S+)\s+(?P<host>\S+)\s+"
    r"(?P<app>\S+)\s+(?P<proc>\S+)\s+(?P<msgid>\S+)\s+(?P<body>.*)$"
)
SYSLOG_3164_RE = re.compile(
    r"^<(?P<pri>\d{1,3})>(?P<time>[A-Z][a-z]{2}\s+\d{1,2}\s+\d{2}:\d{2}:\d{2})\s+"
    r"(?P<host>\S+)\s+(?P<app>[^:\[]+)(?:\[\d+\])?:\s*(?P<body>.*)$"
)


def _normalize_syslog_event(
    raw_text: str, line_number: int, source_file: str
) -> ParsedEvent:
    ...
    if match:
        defaults.update(
            ...
            level=_syslog_level(int(match.group("pri")) % 8),
            host=match.group("host"),
            service=match.group("app"),
        )
    ...
    return normalize_text_event(
        ...
        line_number,
        source_file=source_file,
        source_format=ArtifactFormat.SYSLOG.value,
        defaults=defaults,
    )
...
CRI_RE = re.compile(
    r"^(?P<time>\S+)\s+(?P<stream>stdout|stderr)\s+"
    r"(?P<flag>[FP])\s+(?P<body>.*)$",
    re.DOTALL,
)


def _normalize_container_text_event(
    raw_text: str,
    line_number: int,
    source_file: str,
) -> ParsedEvent:
    ...
    inherited = {
        "timestamp": match.group("time"),
        "level": "ERROR" if match.group("stream") == "stderr" else None,
    }
...
def _normalize_cloud_gateway_event(
    ...
    if len(fields) > 12 and fields[0].lower() in {
        "http",
        "https",
        "h2",
        "grpc",
...
def _normalize_cloudfront_event(
    raw_text: str,
    line_number: int,
    source_file: str,
    ...
    return normalize_text_event(
        ...
        defaults={
            "timestamp": timestamp,
            "level": level_from_http_status(status),
            "request_id": _present_cloudfront_value(data.get("x-edge-request-id")),
            "service": "cloudfront",
            "host": host,
            "endpoint": endpoint,
            "http_status": status,
...
EXPLICIT_SERVICE_RE = re.compile(
    r"\b(?:service(?:[_-]?name)?|app|component)[\s=:" + "\"'" + r"]+",
    re.IGNORECASE,
)
LAMBDA_LIFECYCLE_RE = re.compile(
    r"^(?:START|END|REPORT)\s+RequestId:\s*(?P<request_id>\S+)",
    re.IGNORECASE,
)
LAMBDA_APPLICATION_RE = re.compile(
    r"^(?P<time>\d{4}-\d{2}-\d{2}T\S+)\s+"
    r"(?P<request_id>[A-Za-z0-9-]+)\s+"
    r"(?P<level>TRACE|DEBUG|INFO|NOTICE|WARNING|WARN|ERROR|ERR|SEVERE|"
    r"FATAL|CRITICAL|ALERT|EMERG(?:ENCY)?)\b",
    re.IGNORECASE,
)


def _normalize_message_broker_event(
    raw_text: str,
    line_number: int,
    source_file: str,
) -> ParsedEvent:
    ...
    return normalize_text_event(
        ...
        line_number,
        source_file=source_file,
        source_format=ArtifactFormat.MESSAGE_BROKER.value,
        defaults=defaults,
    )


def _normalize_serverless_text_event(
    raw_text: str,
    line_number: int,
    source_file: str,
) -> ParsedEvent:
    ...
    if lifecycle:
        defaults.update(
            request_id=lifecycle.group("request_id"),
            level="INFO",
        )
    elif application:
        defaults.update(
            timestamp=application.group("time"),
            request_id=application.group("request_id"),
            level=application.group("level"),
        )
    ...
    return normalize_text_event(
        ...
        line_number,
        source_file=source_file,
        source_format=ArtifactFormat.SERVERLESS.value,
        defaults=defaults,
    )
